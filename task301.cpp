...
SDL_Texture* obstacles1Texture= NULL;
SDL_Texture* obstacles2Texture= NULL;
SDL_Texture* obstacles3Texture= NULL;
SDL_Texture* obstacles4Texture= NULL;
...
void renderGame(SDL_Renderer* renderer, const vector<Snake>body, int foodX, int foodY, bool bonusFoodActive, int bonusFoodX, int bonusFoodY, int score, bool gameover){
    ...
    SDL_RenderCopy(renderer, backgroundTexture, NULL, NULL); // Draw the background
        ...
        SDL_Rect obst1 = {0, 0, 20, 600};
        SDL_RenderCopy(renderer, obstacles1Texture, NULL, &obst1);


        SDL_Rect obst2 = {780, 0, 20, 600};
        SDL_RenderCopy(renderer, obstacles2Texture, NULL, &obst2);

        SDL_Rect obst3 = {20, 580, 100, 20};
        SDL_RenderCopy(renderer, obstacles3Texture, NULL, &obst3);

        SDL_Rect obst4 = {680, 0, 100, 20};
        SDL_RenderCopy(renderer, obstacles4Texture, NULL, &obst4);
...
void initializeTextures(SDL_Renderer* renderer) {
    ...
    obstacles1Texture = loadTextureWithErrorCheck(renderer, "photo/obstraY.png");
    obstacles2Texture = loadTextureWithErrorCheck(renderer, "photo/obstraY.png");
    obstacles3Texture = loadTextureWithErrorCheck(renderer, "photo/obstraX.png");
    obstacles4Texture = loadTextureWithErrorCheck(renderer, "photo/obstraX.png");
...
void cleanupTextures() {
    ...
    SDL_DestroyTexture(obstacles1Texture);
    SDL_DestroyTexture(obstacles2Texture);
    SDL_DestroyTexture(obstacles3Texture);
    SDL_DestroyTexture(obstacles4Texture);
}
...
bool checkFoodOnObstacle(int x, int y){
    if(x>=WINDOW_WIDTH-20 || x<20 ) return true;
    if(x<=100 && y>=580 || x>=680 && y<20) return true;
    return false;
}
bool checkCollisionWithObstacle(){
    int nextX = body[0].x + dx;
    int nextY = body[0].y + dy;
    if(nextX>=WINDOW_WIDTH-20 || nextX<20 ) return true;
    if(nextX<=100 && nextY>=580 || nextX>=680 && nextY<20) return true;
    return false;
}
void spawnFood(){
    while (checkFoodOnSnake(foodX, foodY) || checkFoodOnObstacle(foodX, foodY) ) {
        foodX = (rand() % (WINDOW_WIDTH / FOOD_SIZE)) * FOOD_SIZE;
        foodY = (rand() % (WINDOW_HEIGHT / FOOD_SIZE)) * FOOD_SIZE;
    }
}
void spawnBonusFood(){
    while (checkFoodOnSnake( bonusFoodX, bonusFoodY)|| checkFoodOnObstacle(foodX, foodY)) {
